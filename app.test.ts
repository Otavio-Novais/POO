...
import { UserNotExist } from "./errors/user-not-exist-error";
import { DuplicateUser } from "./errors/duplicate-user-error";
import { RentNotFound } from "./errors/rent-not-found-error";
import { DuplicateBike } from "./errors/duplicate-bike-error";
...
describe('App', () => {
    ...
    it('shoud throw user not found error when user is not found', async () => {
        const app = new App()
        expect(async () => {
            await app.authenticateUser('fakeEmail@gmail.com', 'fakePassword')
        }).rejects.toThrow(UserNotFoundError)
    })

    // Rent not found error
    it('should throw rent not found when rent is not found', async () => {
        ...
        expect(await app.returnBike(bike.id, user.email)).rejects.toThrow(RentNotFound)
    })
})
